...
Future<void> deleteFirstAutoProject(String token) async {
  ...
  final deleteQuery = await http.post(
      Uri.parse('$serverIp/graphql'),
      headers: {
        "content-type": "application/json",
        "authorization": "Bearer $token",
        "cookie": "authn=$token"
      },
      body: '{"query": "mutation ActiveUserMutations(\$deleteId: String!) { projectMutations { delete(id: \$deleteId) } }", "variables": {"deleteId": "$firstProjectID"}}');
